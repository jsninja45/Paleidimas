...
class UserDownloadAudioTranscription {
	...
	{
		//$error = false;
		$transcription_id = $request->route()->transcription_id;
		...
		if ($order->whereHas('audios.transcriptions', function($query) use ($transcription_id) {
			$query->where('id', $transcription_id);
		})->exists()) {
			return $next($request);
		}
