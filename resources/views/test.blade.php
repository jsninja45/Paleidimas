...
if (isset($_GET["id"])) {
    ...
    echo '<pre>'; print_r($result); die(' ' . __FILE__ . ':' . __LINE__);
    $result2 = Braintree_Transaction::submitForSettlement($result->transaction->id);
    echo '<pre>'; print_r($result2); die(' ' . __FILE__ . ':' . __LINE__);
}
...
<form method="POST" action="<?php echo Braintree_TransparentRedirect::url() ?>" autocomplete="off">
    ...
    <fieldset>
        ...
        <?php braintree_text_field('Expiration Month', 'transaction[credit_card][expiration_month]', $result); ?>
        <?php braintree_text_field('Expiration Year', 'transaction[credit_card][expiration_year]', $result); ?>
